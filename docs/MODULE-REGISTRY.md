...
## P42 批次 8（2026-09-12 M1 已查收 pass）

回血统一飘字、新跟班「狼群」、战士激发力量三条（斩返/旋风斩/壁垒）、敌方弹体弹反接口、激发力量按角色分池。

| 窗口 | 内容 | 状态 |
|------|------|------|
| M3 | `TASK-052`：回血飘字收进 `player.heal()`（生存飘 / 复活不飘 / 一次一条） | ✅ done（player 176 条） |
| M7 | `TASK-053`：狼群 3 只 × 基础伤 4，与恶魔共用索敌/跟随 | ✅ done（companions 207 条） |
| M2 | `TASK-054`：斩返（范围 +1 加法、弹反 CD 0.5s）/ 旋风斩 / 壁垒（400 次） | ✅ done（combat 259 条） |
| M4 | `TASK-055`：`clearBulletsIn` 按圆清敌方弹体 | ✅ done（enemies 286 条） |
| M6 | `TASK-056`：激发力量按角色分池 + 狼群入池与文案 | ✅ done（ui 483 条） |
| M5 | `TASK-057`：五任务的**独立验收方**（五份 verify-report + 六探针 61 项） | ✅ done |
| M1 | `match.js` 接线（弹反注入 + 回血去重）、素材入库、查收、文档回写、发布 | ✅ done |

---

## P42 批次 7（2026-09-12 M1 已查收 pass）

蝎子弹与冰人冰锥拆分、树木去序列帧、荆棘图标 + 4 帧爆发特效、小金刚「再获得一次」+ 脉冲 3 身位/×2、生生不息档 8 复活（暂不可达）、新高级「贪婪」。

| 窗口 | 内容 | 状态 |
|------|------|------|
| M4 | `TASK-046`：蝎子弹回 `怪物子弹.png`（与冰锥三层独立）+ 灰树回单帧 | ✅ done（enemies 282 条） |
| M5 | `TASK-047`：普通树回单帧 | ✅ done（world 92 条） |
| M2 | `TASK-048`：荆棘 4 帧特效 + 脉冲 3 身位/×2 + 大娃/四娃/五娃/二娃「再获得一次」 | ✅ done（combat 245 条） |
| M3 | `TASK-049`：三娃 +2 层甲 / 六娃时长按层数 +1 + 档 8 复活接口 | ✅ done（player 172 条） |
| M6 | `TASK-050`：贪婪入池与概率/冷却 + 小金刚/生生不息羁绊文本 | ✅ done（ui 474 条） |
| M7 | `TASK-051`：五任务的**独立验收方**（五份 verify-report + 19/19 探针） | ✅ done |
| M1 | 素材核规格入库、`match.js` 接线（贪婪经验翻倍、档 8 复活触发）、查收、文档回写、发布 | ✅ done |

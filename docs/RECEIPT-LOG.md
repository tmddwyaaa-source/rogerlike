...
## 2026-09-12 查收 — ROUND-016 · 批次 8（回血飘字 / 狼群 / 战士激发力量三条 / 弹反接口 / 分池与文案）

用户需求（口径见 `docs/派工-批次8.md`）：① **所有回血都要飘数字**（生存也飘、档 8 复活不飘、一次只飘一条）；② 新普通跟班 **狼群**（3 只 × 基础伤 4、AI 复用小恶魔、计入万物一心）；③ 战士激发力量也**每 10 级四选一**（斩返 / 旋风斩 / 壁垒 / sp），斩返 = 挥砍范围 **+1 身位（加法）** + 弹反子弹（**消失**，CD 0.5s）、旋风斩 = 圆形半径＝挥砍长度 / **70% 只吃暴击与暴伤** / 每敌一次、壁垒 = **400** 次挥砍命中 +1 甲。
用户信号：「六个窗口已完成，请查收」。
...
| 窗口 | 检查项 | 结果 |
|------|--------|------|
| M3 / TASK-052 | 回血飘字收进 `player.heal()`（满血不飘、生存飘、复活不飘） | ✅ exit 0；**player 176 条 PASS** |
| M7 / TASK-053 | 狼群 3 只 × 基础伤 4 + 恶魔同源 AI + 4 帧相位错开 | ✅ exit 0；**companions 207 条 PASS** |
| M2 / TASK-054 | 斩返（范围 +1 加法 / 弹反 CD 0.5s）+ 旋风斩 + 壁垒（400 次） | ✅ 三条 exit 0；**combat 259 条 PASS** |
| M4 / TASK-055 | `clearBulletsIn({x,y,radius})`：按圆清敌方弹体（冰锥+蝎子弹），返回数量、非法参安全 | ✅ exit 0；**enemies 286 条 PASS** |
| M6 / TASK-056 | 激发力量按角色分池（游侠 4 / 战士 4 / 法师 1）+ 狼群入池与万物一心 9 种 | ✅ 两条 exit 0；**ui 483 条 PASS** |
| M5 / TASK-057 | 独立验收方（reviewer=M5 ≠ 施工窗）：五份 verify-report + **六个自写探针 61 项全 PASS** | ✅ 三条命令本窗复跑 exit 0 |
| **M1 接线** | `match.js`：① 斩返弹反——把 M4 的 `foes.clearBulletsIn({x,y,radius})` 注入 combat 的 `hooks.deflectBullets`；② 回血去重——移除蝙蝠链（companions `onHeal` → `match.onHeal(player,n)`）的重复飘字 | ✅ **match.selftest 52 条 PASS** |
| M1 集成 | 7 个 selftest 全量 + 断言标题核对 | ✅ **1555 条 PASS / 0 FAIL**；批次 8 的 34 条标题全部逐字在盘 |
| 收口 | `transition` ×12 + `audit-round` | ✅ **BASIC + FULL PASS + ROUND_READY_TO_CLOSE** |

### 独立验收发现（M5）

- **M5-T057-F054-1（medium，口径回写）**：旋风斩伤害实现是 **`ceil(0.7 × 基础攻击 × 暴击倍率)`**（不含蓄力倍率/大娃），与「只吃暴击和暴击伤害」自洽；M1 已按**实现口径**回写设计表与 SPEC（避免后人按「该次挥砍伤害的 70%」误读）。
- **M5-T057-F054-2（low，口径选择）**：弹反目前清的是**以角色为圆心的整圆**（半径 = 刀身最远边）——**身后的子弹也会被清**，表现上是「转一圈弹反」。manifest 允许圆/近似，M1 按圆接线；若要改成「只在挥砍正面方向弹反」，需加角度过滤（**留给用户拍板**）。
- **M5-T057-F054-3（low）**：旋风斩是否暴击**沿用主发那一刀的判定**（不独立再摇）；与「连射额外发同参数」一致。
- **M5-T057-F052-1（info，重要提醒）**：`heal()` 同时负责「飘字」与「档 8 复活冷却计数」——M1 去重时**只能去掉重复的飘字，不能绕开 `heal()`**（否则复活冷却漏计）。M1 的实现正是移除重复的 `spawnHealNum` 调用，`heal()` 照常被调用 ✓。
- **M5-T057-F053-1 / F053-2（low/info）**：狼群伤害仍是「与狼**重叠**才结算」（三只都贴上目标才有 3 份 DPS）；狼与恶魔**共用** `isDemonLike` 的移动/索敌代码，但 `addWolfPack` **不动 demonCount** → **狼不参与「跟班伤害→角色伤害」联动**（口径已写进设计表）。
- **M5-T057-F055-1（info）**：`clearBulletsIn` 只返回数量、不返回坐标 → 目前无法在子弹位置播弹反特效；若以后要特效需扩展接口。
- **M5-T057-F056-1 / F056-2（low/info）**：战士拿满三个专属后**只剩 sp**（与游侠池同构）；未知 charId 会静默回落游侠池（与既有默认一致）。

### 文档回写（M1）

`游戏当前设计表.md`（文首摘要、§2.4 新增 `wolf_pack` 行、§2.4b 万物一心 8 → **9** 种、§2.4c 按角色分池 + 三条战士效果、§2.5 跟班、§3 回血飘字口径）、`docs/GAME-SPEC.md`（头标、§4.1 狼群、§4.1b 分池与三条效果、§4.2 跟班、§4.3 万物一心 9 种）、`更新日志.md`（新增 **v0.14 → v0.15**）、`docs/MODULE-REGISTRY.md`、`docs/派工-总览.md`。

**结论**：ROUND-016 **pass**，六个任务（TASK-052…057）全部 `done`，本轮闭环。**待用户拍板**：弹反要不要限定「只在挥砍正面方向」而不是整圈。

---

## 2026-09-12 查收 — ROUND-015 · 批次 7（蝎子弹拆分 / 树木去序列帧 / 荆棘特效 / 小金刚改法 / 生生不息档8 / 贪婪）

用户需求（口径已在派工前逐条确认，见 `docs/派工-批次7.md`）：① 冰锥只属于 Boss 冰人，**蝎子怪子弹回归**；② 移除**树木**序列帧；③ 荆棘 / 滋补图标 + 荆棘 **4 帧爆发特效**素材（用户提供）；④ 新增高级 **贪婪**；⑤ **小金刚**强化改「再获得一次」+ 脉冲 **3 身位 / ×2**；⑥ 生生不息新增**档 8**（致命伤 1 血复活，冷却＝回血 5 次；用户明示**当前不可达**，先标效果）。
用户信号：「六个窗已完成，请查收」。
...
| 窗口 | 检查项 | 结果 |
|------|--------|------|
| M4 / TASK-046 | 蝎子弹与冰人冰锥**彻底拆开**（常量/行为/绘制三层）+ 灰树回单帧 | ✅ 两条 exit 0；**enemies 282 条 PASS** |
| M5 / TASK-047 | 普通树（浅树）回单帧 | ✅ exit 0；**world 92 条 PASS** |
| M2 / TASK-048 | 荆棘 4 帧特效（居中 / 随半径 / 播完消失）+ 脉冲 3 身位 / ×2 + 大娃·四娃·五娃·二娃「再获得一次」 | ✅ 三条 exit 0；**combat 245 条 PASS** |
| M3 / TASK-049 | 三娃 +2 层甲、六娃失锁时长按层数 +1 + 档 8 复活接口（5 次真实回血冷却） | ✅ 两条 exit 0；**player 172 条 PASS** |
| M6 / TASK-050 | 贪婪（50%/层 +10%/上限 100%/0.1s 冷却/不入羁绊）+ 小金刚与生生不息羁绊文本 | ✅ 两条 exit 0；**ui 474 条 PASS** |
| M7 / TASK-051 | 独立验收方（reviewer=M7 ≠ 施工窗）：五份 verify-report + 自写探针 19/19 | ✅ 11 条命令本窗复跑 exit 0 |
| **M1 接线** | `match.js`：① 贪婪——`onCrystal` 调 `session.rollGreedBonus()` 命中就 `notifyExp(2)`；② 档 8——`onHurt` 里「档位 ≥ 8 且 hp ≤ 0」时 `player.tryRevive()`（必须在死亡流程之前把 hp 拉回 1） | ✅ **match.selftest 52 条 PASS** |
| M1 集成 | 7 个 selftest 全量 + 断言标题核对 | ✅ **1519 条 PASS / 0 FAIL**；批次 7 的 35 条标题全部逐字在盘 |
| 收口 | `transition` ×12 + `audit-round` | ✅ **BASIC + FULL PASS + ROUND_READY_TO_CLOSE** |

### 独立验收发现（M7）

- **M7-T046-F1（info）**：蝎子弹与冰锥在**常量层 / 行为层 / 绘制层**三层都独立——蝎子 `kind=scorpion`、无 rotate；冰人弹幕 `kind=ice`、有尖头补偿 rotate。批次 6 的串台确认修好。
- **M7-T046-F2 / T047-F1（info）**：灰树 / 浅树「回单帧」确认——代码里不再引用 `*-2/-3/-4`（素材按口径留在磁盘），只出现在注释里；M7 探针首版把「注释里提到旧文件名」误判成「还在引用」，去注释后重测通过。
- **M7-T048-F2（low，交付后已消除）**：M2 施工**半改中间态**期间，`combat` 仍 import 已被删除的 `FIRE_DMG_PER_PICK_BOOST` → ESM link error、整模块加载失败（M7 首跑撞上）。M2 交付后 M7 按最终磁盘状态**重跑 13 条命令（全 exit=0）+ 探针 19/19**。
- **M7 探针自纠（info，值得记）**：M7 的探针首版 `check()` **没有 await 异步用例** → 异步断言会**假 PASS**（荆棘特效那条实际 `sheets.size=0` 却记成 PASS）；改成顺序 await 后才暴露。**教训：验渲染类改动必须 await 异步加载，否则探针会骗自己。**
- **M7-T049-F1 / T050-F1（info）**：三娃「有金刚 1 次 == 无金刚 2 次」（+2 层甲）、复活「就绪→hp=1→消耗→5 次真实回血后恢复」、`greedChance(0/1/2/3/6/99) = 0/50/60/70/100/100`（封顶正确）、`rollGreedBonus` 0.1s 冷却、贪婪四个羁绊计数全 0 且 `tier=advanced`——逐条实测通过。

### 文档回写（M1）

`游戏当前设计表.md`（文首摘要、§2.4 新增 `greed` 行与高级项 9 → **10**、§2.4b 小金刚与生生不息、§6 序列帧第二批与冰锥归属、§2.4 荆棘特效）、`docs/GAME-SPEC.md`（头标、§4.1 贪婪、§4.3 小金刚/生生不息、§5 动画与冰锥归属）、`更新日志.md`（新增 **v0.13 → v0.14**）、`docs/MODULE-REGISTRY.md`、`docs/派工-总览.md`。

**结论**：ROUND-015 **pass**，六个任务（TASK-046…051）全部 `done`，本轮闭环。

---

## 2026-09-12 查收 — ROUND-014 · 批次 6（序列帧第二批 + 冰锥）+ 修补（定神 / 暴击倍率 / 伤害取整 / 文案）

用户需求两块：
1. **批次 6**：桌面 `Roger-png` 的 `boos`（冰人 **6 帧** + 冰锥）、`跟班`（兔子 / 史莱姆g-1 / g-2 / 蛋-x·y·z / 恶魔 / 蝙蝠 各 4 帧）、`树木`（灰树 / 浅树 各 4 帧）序列帧；**子弹尖头朝飞行方向**；`子弹` 目录其余**不替换**。
2. **修补**（实机反馈）：定神站着不动完全吃不到 +100 暴击；暴击倍率澄清「向上取整指的是**伤害数字**」；飘字「×倍率」要**两位小数**。
用户信号：「批次 6 的窗口们都已完成，请查收」→（M3 的独立验收尚未交）→「M3、M6 已完成，请查收」。

### 磁盘核对 + 本窗重跑（M1）

| 窗口 | 检查项 | 结果 |
|------|--------|------|
| M4 / TASK-038 | 冰人 **6 帧**（0.6s 一轮）+ 灰树 **4 帧** + 冰锥换图并**尖头朝飞行方向** | ✅ 三条 exit 0；**enemies 283 条 PASS** |
| M7 / TASK-039 | 8 种跟班各 4 帧（蛋三阶段各自一套） | ✅ exit 0；**companions 202 条 PASS** |
| M5 / TASK-040 | 普通树（浅树）4 帧 | ✅ exit 0；**world 98 条 PASS** |
| M3 / TASK-041 | 批次 6 的**独立验收方**（reviewer=M3 ≠ M4/M7/M5）：三份 verify-report + 70 条探针 | ✅ 五条 verify_cmd 本窗逐字复跑 exit 0；同种子双模拟证明动画不改判定 |
| M2 / TASK-042 | 暴击倍率**连续算式**（去 `ceil(率/3)`）+ 所有伤害结算前 `Math.ceil` | ✅ 三条 exit 0；**combat 239 条 PASS** |
| M3 / TASK-043 | 定神打断条件**只留 WASD**（蓄力/攻击/受击不打断）+ 飘字标签两位小数 + **真实输入路径断言** | ✅ 两条 exit 0；**player 160 条 PASS** |
| M6 / TASK-044 | 修补的独立验收方（真实输入路径探针） | ✅ pass；报 1 条 low（desc 措辞） |
| M6 / TASK-045 | 精益求精 desc 去掉「（向上取整）」措辞 | ✅ exit 0；**ui 464 条 PASS** |
| M1 集成 | 7 个 selftest 全量 + 断言标题核对 | ✅ **1498 条 PASS / 0 FAIL**；批次 5 的 34 条 + 修补的 13 条 + TASK-045 的 2 条标题全部逐字在盘 |
| 收口 | `transition` ×12 + `audit-round` | ✅ **BASIC + FULL PASS + ROUND_READY_TO_CLOSE** |

### 独立验收发现

- **M3-F-HITBOX（low，批次 6）**：冰锥贴图 12px、判定盒仍 4px → 「看着擦边没中 / 看着没碰到却掉血」概率变大。**属换图的观感副作用，判定没被改**；本轮不回工。
- **M3-F-OFF（low，批次 6）**：冰锥源图主体不在画布正中（偏移 3px，缩放后约 1.1px）→ 旋转绘制时主体会小幅画圈。可让 M10/M4 重出居中版；本轮不回工。
- **M3-F-NEG（info）**：取帧兜底口径三模块不一致（enemies 夹到 0；companions/world 回绕）——当前调用点恒传非负 number，**不可达**，留档。
- **M6-T042-F1（low，已修）**：玩家可见的「精益求精」desc 仍写「（向上取整）」→ 已开 `TASK-045` 改掉并更新断言。
- **M6-T042-F2 / F3、M6-T043-F1 / F2（info）**：定神 +100 在**没拿精益求精**时只保必暴、不加伤害（连续式含 ×层数，picks=0 恒 1.5）；伤害取整是纯 `Math.ceil`（不做浮点 epsilon 兜底）；探针首版误报均属探针自身缺陷（假 camera 覆盖朝向、用非必暴的 9% 率当确定性用例）。
- **流程（M1 已修）**：`TASK-041` 的 `worker-report` 有三处不合门禁：① `changed_files` 是空数组，而它 `allowed_paths` 内新增了 19 个文件（三份 verify-report + evidence）→ 门禁判「越界未申报」；② `tests[]` 里有一条**给 M1 的说明文字**（`py -3 ... transition TASK-038/039/040 verifying|verified`），含 `|` 被 `cmd.exe` 当管道执行 → exit 255；③ 两条命令带 `#` 注释。M1 收口时补申报、删掉那条指令文字、去掉注释（命令本体未改）。**纪律再补一句：`tests[]` 只放可执行命令，指令/说明写 `notes`。**

### 文档回写（M1）

`游戏当前设计表.md`（文首摘要、§2.5 跟班 4 帧口径、§6 序列帧第二批与冰锥朝向、暴击倍率与取整/两位小数、定神静止判定）、`docs/GAME-SPEC.md`（头标、§2 暴击与取整、§4.1 精益求精、§4.1b 定神、§4.2 跟班、§5 小怪动画与冰锥）、`更新日志.md`（v0.13 增加「序列帧第二批」与定神修复）、`docs/MODULE-REGISTRY.md`、`docs/派工-总览.md`。

**结论**：ROUND-014 **pass**，八个任务（TASK-038…045）全部 `done`，本轮闭环。
